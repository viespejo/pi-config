 ...
 * (right) pane:
 *   gda — Allow (guardian-accept: saves edits, closes diff)
 *   gdr — Block (guardian-reject: closes diff, rejects change)
 *
...
async function reviewInEmbeddedNvim(
  ...
  const setupLua = `
    local current_file = [[${luaStr(currentFile)}]]
    local proposed_file = [[${luaStr(proposedFile)}]]
    ...
    vim.keymap.set('n', 'gda', function() ${luaAllowFn} end, { buffer = prop_buf, desc = 'Guardian: Allow' })
    vim.keymap.set('n', 'gdr', function() ${luaBlockFn} end, { buffer = prop_buf, desc = 'Guardian: Block' })
  `;

  nvimRemoteSend(
    nvimServer,
    `<C-\\><C-n>:lua ${setupLua.replace(/\n/g, " ")}<CR>`,
  );

  // Watch for decision from Neovim keybind
  const { promise: nvimDecision, cancel: cancelWatch } = watchForDecision(
    tmpDir,
    decisionFile,
  );
  ...
  const tuiPromise: Promise<RaceResult> = selectWithNotification(
    ctx,
    `${fileName} — diff in Neovim  [gda = allow, gdr = block]`,
