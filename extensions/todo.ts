...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  private updateHints(): void {
    this.hintText.setText(
      this.theme.fg(
        "dim",
        "Type to search • ↑↓ select • Enter actions • Ctrl+W work • Ctrl+R refine • Esc close • /todos add <title> • /todos help",
      ),
    );
  }
  ...
  handleInput(keyData: string): void {
    ...
    if (matchesKey(keyData, Key.ctrl("w"))) {
      const selected = this.filteredTodos[this.selectedIndex];
...
const TODOS_COMMAND_USAGE =
  "Usage:\n" +
  "  /todos                  Open todo manager (or print list in non-UI mode)\n" +
  "  /todos add <title> [--tags a,b] [--status open] [--body \"details\"] [--claim]\n" +
  "  /todos create <title> ... (alias of add)\n" +
  "  /todos new <title> ... (alias of add)\n" +
  "  /todos help";

interface ParsedTodosCreateCommand {
  title: string;
  tags: string[];
  status?: string;
  body?: string;
...
function parseTodosCreateCommand(tokens: string[]): ParsedTodosCreateCommand {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    const option = token.slice(2).toLowerCase();
    if (option === "claim") {
      claim = true;
      continue;
    }
  ...
  if (!title) {
    return { title: "", tags: [], status, body, claim, error: "title required" };
  }
...
export default function todosExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("todos", {
    description:
      "Manage todos: open manager/list, add/create/new todos, and show help usage", 
    handler: async (args, ctx) => {
      const todosDir = getTodosDir(ctx.cwd);
      const rawArgs = (args ?? "").trim();

      const printUsage = () => {
        if (ctx.hasUI) ctx.ui.notify(TODOS_COMMAND_USAGE, "info");
        else console.log(TODOS_COMMAND_USAGE);
      };
      ...
      if (rawArgs) {
        ...
        if (
          ...
          const parsed = parseTodosCreateCommand(tokens.slice(1));
          if (parsed.error) {
            const message = `Error: ${parsed.error}`;
            if (ctx.hasUI) ctx.ui.notify(`${message}\n${TODOS_COMMAND_USAGE}`, "error");
            else {
              console.error(message);
              console.log(TODOS_COMMAND_USAGE);
            }
            return;
          }
          ...
          const created = await withTodoLock(todosDir, id, ctx, async () => {
            await writeTodoFile(filePath, todo);
            return todo;
          });

          if (typeof created === "object" && "error" in created) {
            if (ctx.hasUI) ctx.ui.notify(created.error, "error");
            else console.error(created.error);
            return;
          }

          let createdTodo = created as TodoRecord;
          if (parsed.claim) {
            const claimResult = await claimTodoAssignment(todosDir, id, ctx);
            if (typeof claimResult === "object" && "error" in claimResult) {
              if (ctx.hasUI) ctx.ui.notify(claimResult.error, "warning");
              else console.error(claimResult.error);
            } else {
              createdTodo = claimResult as TodoRecord;
            }
          }

          if (ctx.hasUI) {
            ctx.ui.notify(
              `Created ${formatTodoHeading(createdTodo)}${parsed.claim ? " (claimed)" : ""}`,
              "info",
            );
          } else {
            console.log(serializeTodoForAgent(createdTodo));
          }
          return;
        }
      }
