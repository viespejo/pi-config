...
const showActionSelector = async (
...
): Promise<"reveal" | "open" | "edit" | "addToPrompt" | "diff" | null> => {
  const actions: SelectItem[] = [
    ...(options.canDiff ? [{ value: "diff", label: "Diff in Neovim" }] : []),
    { value: "reveal", label: "Reveal (Oil/nvim)" },
  ...
  return ctx.ui.custom<
    "reveal" | "open" | "edit" | "addToPrompt" | "diff" | null
  >((tui, theme, _kb, done) => {
    ...
    selectList.onSelect = (item) =>
      done(item.value as "reveal" | "open" | "edit" | "addToPrompt" | "diff");
...
const getPreferredEditorCommand = (): string => {
  if (process.env.PI_FILES_EDITOR) {
    return process.env.PI_FILES_EDITOR;
  }
...
const isPiEditorExecutable = (executable: string): boolean =>
  path.basename(executable).toLowerCase() === "pi-editor";
...
const openExternalEditor = (
  ...
  tui.stop();
  try {
    const { executable, args } = splitCommand(editorCmd);
    const editorArgs = isPiEditorExecutable(executable)
      ? ensurePiEditorPlainModeArgs(args)
      : args;
    const result = spawnSync(executable, [...editorArgs, targetPath], {
      stdio: "inherit",
    });
...
const openExternalDiff = (
  ...
  if (isPiEditorExecutable(executable)) {
    ...
    diffArgs.push("--diff", originalFile, workingFile);
  } else {
    diffArgs.push(originalFile, workingFile);
  }

  tui.stop();
  try {
    const result = spawnSync(executable, diffArgs, {
      stdio: "inherit",
    });
    return result.status === 0;
  } finally {
...
const editPath = async (
  ...
  const opened = await ctx.ui.custom<boolean>((tui, theme, _kb, done) => {
  ...
  if (!opened) {
    ctx.ui.notify(`Editor closed (non-zero exit): ${target.displayPath}`, "warning");
  }
};

type RevealAttemptResult = {
  opened: boolean;
  usedFallback: boolean;
};

const buildRevealArgs = (
  executable: string,
  args: string[],
  ...
  return [...baseArgs, targetDirectory];
};

const openExternalReveal = (
  tui: TUI,
  targetDirectory: string,
): RevealAttemptResult => {
  const configuredEditor = process.env.PI_FILES_EDITOR?.trim() || null;
  const attempts: Array<{ command: string; useOilFallback: boolean }> = [];

  if (configuredEditor) {
    attempts.push({ command: configuredEditor, useOilFallback: false });
  }

  if (commandExists("pi-editor")) {
    attempts.push({ command: "pi-editor --mode plain", useOilFallback: true });
  }

  attempts.push({ command: "nvim", useOilFallback: true });

  const seen = new Set<string>();
  const orderedAttempts = attempts.filter((attempt) => {
    if (seen.has(attempt.command)) {
      return false;
    }
  ...
  tui.stop();
  try {
    for (let i = 0; i < orderedAttempts.length; i += 1) {
      const attempt = orderedAttempts[i];
      const { executable, args } = splitCommand(attempt.command);
      const revealArgs = buildRevealArgs(executable, args, targetDirectory);
      const result = spawnSync(executable, revealArgs, { stdio: "inherit" });

      if (result.status === 0) {
        return { opened: true, usedFallback: i > 0 };
      }

      if (!attempt.useOilFallback) {
        continue;
      }
    }
...
const revealPath = async (
  ...
  const statusLabel = process.env.PI_FILES_EDITOR
    ? "PI_FILES_EDITOR"
    : commandExists("pi-editor")
      ? "pi-editor --mode plain"
      : "nvim";

  const revealResult = await ctx.ui.custom<RevealAttemptResult>(
    (tui, theme, _kb, done) => {
      const status = new Text(
        theme.fg("dim", `Opening reveal in ${statusLabel}...`),
      );

      queueMicrotask(() => {
        done(openExternalReveal(tui, targetDirectory));
      });
  ...
  if (
    process.env.PI_FILES_EDITOR &&
    revealResult.opened &&
    revealResult.usedFallback
  ) {
    ctx.ui.notify(
      "PI_FILES_EDITOR failed; falling back to pi-editor/nvim",
      "warning",
    );
  }

  if (!revealResult.opened) {
...
const openDiff = async (
  ...
  try {
    ...
    const opened = await ctx.ui.custom<boolean>((tui, theme, _kb, done) => {
      const status = new Text(
        theme.fg("dim", `Opening diff in ${editorCmd}...`),
      );

      queueMicrotask(() => {
        const result = openExternalDiff(tui, editorCmd, tmpFile, workingPath);
        done(result);
      });
...
const showFileSelector = async (
  ...
  quickAction: "diff" | "actions" | "addToPrompt" | "open" | "reveal" | null;
}> => {
    ...
    (tui, theme, keybindings, done) => {
      ...
      container.addChild(
        new Text(
          theme.fg(
            "dim",
            "Type filter • enter default action • alt+d diff • alt+a actions • alt+p prompt • alt+o open • alt+r reveal • esc cancel",
          ),
          0,
          0,
        ),
        ...
        handleInput(data: string) {
          if (matchesKey(data, "alt+d")) {
            ...
            if (selected) {
              ...
              if (!canDiff) {
                ctx.ui.notify(
                  "Diff requires a tracked file (exists in git history)",
                  "warning",
                );
                return;
              }
...
const runFileBrowser = async (
  ...
  while (true) {
    ...
    if (quickAction === "reveal") {
      await revealPath(ctx, selected);
      continue;
    }

    if (quickAction !== "actions") {
